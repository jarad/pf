...
sir_pmcmc <- function(n.chain, niter, np, y.max)
{
  # Create pomp object
  y = y[,1:y.max]
  sir.pomp <- pomp(data = y, times = 1:dim(y)[2], t0 = 0, P=P, b=b, varsigma=varsigma, sigma=sigma, eta = eta, rprocess=rprocess, rmeasure=rmeasure, dmeasure=dmeasure, skeleton = skeleton, skeleton.type = "map", initializer=initializer, rprior=rprior.pomp, dprior=dprior)
  ...
  file = paste(dpath,"sir_pmcmc_test-hsd-",paste(n.chain,niter,np,y.max,sep="-"),".rdata",sep="")
  save(out, file = file)
  print(time)
}
require(plyr)
require(doMC)
registerDoMC()
mydata = expand.grid(n.chain=1:3, niter=20000, np=100, y.max = c(30,60,90,125))
