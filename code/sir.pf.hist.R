...
pdf(paste(gpath,"Hist-Time-KD-",p,"P-",n,"-betagamma-scatter.pdf",sep=""),width=10,height=5)
par(mfrow=c(2,4))
for(i in 1:length(cutoff))
{
  if(i == 1)
...
for(i in 1:length(cutoff))
{
  a = bkde2D(cbind(xrwb[,i],xrwg[,i]),h)
  xmin = min(xmin,a$x1)
  xmax = max(xmax,a$x1)
  ymin = min(ymin,a$x2)
  ymax = max(ymax,a$x2)
...
for(i in 1:length(cutoff))
{
  a = bkde2D(cbind(xrwb[,i],xrwg[,i]),h,range.x=list(x1=c(xmin,xmax),x2=c(ymin,ymax)))
  if(i==1){
    image(a$x1,a$x2,a$fhat,xlim=c(xmin,xmax),ylim=c(ymin,ymax),
        main=paste("k = ",cutoff[i]-1,sep=""),xlab=expression(beta),
    ...
    image(a$x1,a$x2,a$fhat,xlim=c(xmin,xmax),ylim=c(ymin,ymax),xlab="",ylab="",
        main=paste("k = ",cutoff[i]-1,sep=""),cex.main=msize,cex.lab=labsize)
    contour(a$x1,a$x2,a$fhat,add=TRUE)
    points(theta[1],theta[2],pch=20,col=4)
  }
}
