...
for(i in 1:length(ns))
{
  for(k in 1:length(params))
  {
    ...
    abline(h=theta[k],lty=2)
    if(k == 1 & i == 1) # add legend
    {
      legend("topright",legend=c("Truth","BF","APF","KD"),col=c(1,cols),lty=c(2,1,1,1),cex=cex.leg)
    }
  }
}
dev.off()
...
for(k in 1:length(params))
{
  ...
  if(k %in% 1:3)
  {
    load(paste(dpath,"PF-quant-2-KD-normal-stratified-10000.rdata",sep=""))
    out = pf.quant.out$theta.quant
    tt = dim(out)[1]; nt = tt - 1
