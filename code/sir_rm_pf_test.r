...
plot(0:nt, state.quant.kd[,1,1], type="l", ylim=c(0,1), col = 3)
...
par(mfrow=c(1,2))
plot(0:nt, theta.quant.kd[,1,1], type = "l", col = 3, ylim = c(min(theta.quant.kd[,1,],theta.quant.rm[,1,]),max(theta.quant.rm[,1,],theta.quant.rm[,1,])))
...
plot(0:nt, theta.quant.kd[,2,1], type = "l", col = 3, ylim = c(min(theta.quant.kd[,2,],theta.quant.rm[,2,]),max(theta.quant.kd[,2,],theta.quant.rm[,2,])))
