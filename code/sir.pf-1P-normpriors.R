...
theta.mean = c(-1.3296, -2.1764, 0.1055)[p]
theta.sd = sqrt(c(0.1055, 0.0140, 0.0064))[p]
rtheta = function(){ rnorm(1,theta.mean,theta.sd)}
...
revo_bf = function(x){ revo(x, P, d, s*exp(x[3])+sinv*theta)}
rprior_bf = function()
{ 
...
pstate_apf = function(x) { revo(x, P, d, s*exp(x[3])+sinv*theta, FALSE)}
revo_apf = function(x){ revo(x, P, d, s*exp(x[3])+sinv*theta)}
rprior_apf = function()
{ 
  myprior = rprior(sim$y[,1], rtheta, b, varsigma, sigma, dpower)
  return(c(myprior$x,myprior$theta))
...
pstate_kd = function(x, mytheta) { revo(x, P, d, s*exp(mytheta)+sinv*theta, FALSE)}
revo_kd = function(x, mytheta){ revo(x, P, d, s*exp(mytheta)+sinv*theta)}
