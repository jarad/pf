...
\noindent where $U_{[a,b]}(.)$ is the pdf of a uniform distribution on the interval $[a,b]$ and $\ln N(.;\mu,\sigma^2)$ is the pdf of a log-normal distribution with mean $\mu$ and variance $\sigma^2$ on the log scale.  $p(i_0,s_0)$ is the joint pdf of the random variables $i_0$ and $s_0$ defined by
