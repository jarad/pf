...
Figure \ref{fig:ext} shows 95\% credible intervals over time for marginal filtered distributions of $\theta$ and $x$ for both the extended model and the original model that assumes $b$, $\varsigma$, and $\sigma$ known.  The intervals for $\beta$, $\gamma$, $\nu$, $s$, $i$, and $r$ are wider for the extended model (green lines) than for the original model.  This is expected due to the added uncertainty in $b$, $\varsigma$, and $\sigma$ in the extended model.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{PF-ext-KD-stratified-normal-20000}
\caption{Displays 2.5\%/97.5\% quantiles of the filtered distributions of the unknown states and parameters of the extended model (green) over time compared with that of the model assuming $b$, $\varsigma$, and $\sigma$ known.  $J = 20000$ particles for the KD particle filter were used with normal priors on $\theta$ and stratified resampling.  Tick marks are shown along the bottom of the plots at time points when no resampling was performed in the particle filter.} \label{fig:ext}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{sim-y}
\caption{Displays the exponential of the observed syndromic data over time, simulated from the model described in section \ref{sec:sim}.  Different colors represent data from different syndromes.} \label{fig:y}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{sim-y-ext}
\caption{Displays the exponential of the observed syndromic data over time, simulated from the extended model described in section \ref{sec:extend}} \label{fig:yext}
\end{figure}
