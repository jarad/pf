...
For a comparison with our results using the KDPF, a full MCMC analysis was run using the particle MCMC (PMCMC) approach of \citet{Andr:Douc:Hol:pmcmc:2010}. Using the {\tt pmcmc} function within the R package {\tt pomp} \citep{pomp}, samples from the posterior distribution of the fixed parameters were generated conditional on the first $T$ observations from the simulated data set pictured in Figure \ref{fig:data} for $T = 5, 10, 15, \ldots, 125$. For each $T$, three chains consisting of 20000 PMCMC iterations (i.e. 60000 total samples) were generated and 95\% credible intervals were calculated from the combined samples among the three chains. PMCMC chains were initialized at the true values of the fixed parameters used for simulating the data and random-walk standard deviations for the proposal distributions of $\beta$, $\gamma$, and $\nu$ were set to 0.005, 0.001, and 0.01, respectively. For more information on PMCMC and using functions within the {\tt pomp} package, we refer the reader to \citet{Andr:Douc:Hol:pmcmc:2010} and \citet{pomp}.

Figure \ref{fig:mcmc} compares the PMCMC results with 20 separate runs of the KDPF with 20000 particles on the same simulated data using our log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$. Multiple runs of the KDPF on the same data allows us to assess the uncertainty in the 95\% credible intervals obtained from the filtered distributions at each $T$. For instance, the high variance of the interval estimates for $\nu$ in the rightmost panel of the figure demonstrates the challenge in estimating this parameter. Nonetheless, the performance of the KDPF compares well with PMCMC, as the lower and upper bounds of the 95\% credible intervals obtained from PMCMC over the course of the epidemic appear to cover the range of lower and upper bounds obtained from the separate KDPF runs. Furthermore, the PMCMC on the full data set took 8-9 hours to run while the KDPF with $J = 20000$ particles provided results in about 15 minutes.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{sir-pmcmc-kdpf-1-20000-20-20000}
\caption{Sequential 95\% credible intervals obtained every fifth day for 20 different runs of the KDPF (green lines) compared with 95\% credible intervals obtained from PMCMC (black circles).} \label{fig:mcmc}
\end{figure}
...
The KDPF provides a generic sequential inferential strategy that reduces particle degeneracy in many situations. However, the algorithm has weaknesses and is known to perform poorly under certain conditions. For instance, the KDPF does not perform well in high-dimensional parameter space. In addition, the theoretical justification of particle filters given in Section \ref{sec:just} only holds for fixed $T$. Thus, the KDPF will eventually degenerate if run over long periods of time due to the accumulation of approximation errors. Also, as mentioned in Section \ref{sec:pf}, all SMC methods suffer from degeneracy if vague priors are used. A common solution to this problem is to first run an MCMC based on the first few data points to find a reasonable particle cloud from which to draw prior samples \citep{petris2009dynamic}.

Advanced techniques exist that are better than the KDPF at fighting particle degeneracy, but require more practitioner input. For example, particle degeneracy could be combated within an SMC algorithm by incorporating a MCMC step to refresh fixed parameter values \citep{Gilk:Berz:foll:2001,Stor:part:2002}. However, this would require the practitioner to define a MCMC algorithm in addition to the SMC algorithm. In addition to this requirement, the algorithm would no longer be truly sequential as the computational effort increases with time. Alternatively, if the practitioner is willing to modify their model, they can take advantage of a sufficient statistic structure \citep{Fear:mark:2002}, Rao-Blackwellization \citep{Douc:Gods:Andr:on:2000}, or both \citep{carvalho2010particle}. Possible modifications to the model in Section \ref{sec:model} to allow alternative strategies include setting $\nu=1$, removing fixed parameters from $Q$, and eliminating the truncation in equation \eqref{eqn:state}. At present, the KDPF is still the only particle filtering algorithm that is truly sequential and flexible enough to be applied to any model.

PMCMC methods provide better estimates of unknown states and fixed parameters in high dimensions by using SMC methods to construct efficient proposal distributions. Since it is likely that more complicated models than what we presented in this paper may be required for monitoring disease outbreaks in real-life situations, we recommend that PMCMC be used as long as the time-scale of the epidemic allows for it. However, a sequential analysis could be valuable for processing data collected on a time scale of hours, minutes, or seconds. In these situations, we recommend running SMC and PMCMC algorithms concurrently, as in \citet{petris2009dynamic}, by re-initializing the SMC using posterior samples from PMCMC once it has finished running. This helps avoid degeneracy of particle samples by putting a fixed time limit on the length of particle filter runs while also ensuring that reasonable starting values are used as prior samples for the next particle filter run.
