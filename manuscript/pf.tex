...
Figure \ref{fig:mcmc} compares the results of the PMCMC analyses with 20 separate runs of the KDPF on the same simulated data using our log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$. Multiple runs of the KDPF on the same data allows us to assess the uncertainty in the 95\% credible intervals obtained from the filtered distributions at each $T$. We can clearly see the challenge in estimating $\nu$, exhibited by the variance in the interval estimates for that parameter relative to the more stable estimates of $\beta$ and $\gamma$. The PMCMC results are similar, as the lower and upper bounds of the 95\% credible intervals obtained from the PMCMC fall within the cloud of intervals obtained from the separate KDPF runs for most of the epidemic. Yet, while the PMCMC on the full data set took 8-9 hours to run, the KDPF with $J = 20000$ particles provided results in about 15 minutes.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{sir-pmcmc-kdpf}
\caption{Sequential 95\% credible intervals obtained every fifth day over 20 runs of the KDPF (green lines) using log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$ compared with 95\% credible intervals obtained from separate PMCMC analyses (pink circles) on observations from the same simulated data set observed up until time $T$ for $T = 5, 10, 15, \ldots, 125$.} \label{fig:mcmc}
\end{figure}
