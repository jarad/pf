...
\caption{Displays the sample correlation coefficient between $\beta$ and $\gamma$ over time for the different prior distributions and transformations applied to $\theta$.  For each time $t$, values of $\beta$ and $\gamma$ were taken from 500 particles sampled from the weighted sample approximation of $p(x_t,\theta|y_{1:t})$} \label{fig:corr}
\end{figure}
