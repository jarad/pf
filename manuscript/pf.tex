...
For a comparison with our results using the KDPF, a full MCMC analysis was run using the particle MCMC (PMCMC) approach of \citet{Andr:Douc:Hol:pmcmc:2010}. Using the {\tt pmcmc} function within the R package {\tt pomp} \citep{pomp}, samples from the posterior distribution of the fixed parameters were generated conditional on the first $T$ observations from the simulated data set pictured in Figure \ref{fig:data} for $T = 5, 10, 15, \ldots, 125$. For each $T$, three chains consisting of 30000 PMCMC iterations were generated and 95\% credible intervals were calculated for each chain. PMCMC chains were initialized at the true values of the fixed parameters used for simulating the data. Random-walk standard deviations for the proposal distributions of $\beta$, $\gamma$, and $\nu$ were set to 0.005, 0.001, and 0.01, respectively. These values were chosen such that the chains mixed well within reasonable computing time, but it is possible that different values could provide better mixing, leading to better estimates of fixed parameters within the same computing time. For more information on PMCMC and using functions within the {\tt pomp} package, we refer the reader to \citet{Andr:Douc:Hol:pmcmc:2010} and \citet{pomp}.

Figure \ref{fig:mcmc} compares 95\% credible intervals for each fixed parameter obtained from PMCMC with those produced by 20 separate runs of the KDPF with 20000 particles on the same simulated data using our log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$. Multiple runs of the KDPF on the same data and multiple PMCMC chains allows us to assess the uncertainty in the 95\% credible intervals obtained from the filtered distributions at each $T$. For instance, the high variance of the interval estimates for $\nu$ in the rightmost panel of the figure demonstrates the challenge in estimating this parameter. Nonetheless, the performance of the KDPF compares well with PMCMC, as the lower and upper bounds of the 95\% credible intervals obtained from the PMCMC chains over the course of the epidemic appear to be more variable than the lower and upper bounds obtained from the separate KDPF runs. Furthermore, the PMCMC on the full data set took 8-9 hours to run while the KDPF with $J = 20000$ particles provided results in about 15 minutes.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{sir-pmcmc-kdpf-3-30000-20-20000}
\caption{Sequential 95\% credible intervals obtained every fifth day for 20 different runs of the KDPF (green lines) compared with 95\% 3 different PMCMC chains (black circles).} \label{fig:mcmc}
\end{figure}
