...
\noindent The KDPF adds the kernel density regeneration to the auxiliary particle filter. Here, we use a normal kernel, where $\mbox{N}(\mu,\Sigma)$ represents the pdf of normal distribution with mean $\mu$ and covariance matrix $\Sigma$.
...
PMCMC methods provide better estimates of unknown states and fixed parameters in high dimensions by using SMC methods to construct efficient proposal distributions \citep{Andr:Douc:Hol:pmcmc:2010}. Since it is likely that more complicated models than what we presented in this paper may be required for monitoring disease outbreaks in real-life situations \citep{Sham:Kars:pnas:2012, Bhad:Ioni:mala:2011}, we recommend that PMCMC be used as long as the time-scale of the epidemic allows for it. However, a sequential analysis could be valuable for processing data collected on shorter time scales when an immediate decision regarding an intervention policy is needed \citep{Merl:John:Gram:Mang:stat:2008, ludkovski2010optimal, dukic2012tracking}. In these situations, a possible strategy could involve running SMC and PMCMC algorithms concurrently by re-initializing the SMC using posterior samples from PMCMC once it has finished running \citep{petris2009dynamic}. This helps avoid degeneracy of particle samples by putting a fixed time limit on the length of particle filter runs while also ensuring that reasonable starting values are used as prior samples for the next particle filter run.
