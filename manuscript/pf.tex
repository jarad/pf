...
Notice from equation \eqref{eqn:state} that the transition density needed for prediction in our model is a multivariate normal density with mean $f_\tau(x_t,\theta)$ and covariance matrix $Q$.  A few notes on how to carry out the prediction step for this density are necessary. First, recall that that the mean function approximates the disease dynamics better for small $\tau$.  Thus, if the time between observations, $t+1 - t_k$, is large, we can still set $\tau$ to be small and carry out the prediction step $(t+1 - t_k) / \tau$ times to generate $x_{t+1}^{(j)}$.  Second, $i$ and $s$ are proportions and thus constrained to be in $[0,1]$.  However, the stochastic nature of the state equation makes it possible to generate states that do not satisfy this constraint.  In our implementation of the particle filter, we throw away samples in which $i$ or $s$ fall outside of $[0,1]$ and resample.
...
\[ \tilde{g}_{t+1}^{(j)} = w_k^{(j)} p(y_{t+1}|\mu_{t+1}^{(j)}) \qquad g_{t+1}^{(j)} = \tilde{g}_{t+1}^{(j)}\left/\sum_{l=1}^J \tilde{g}_{t+1}^l\right. \]
...
The kernel density particle filter, we move to an approximation of $p(x_{t+1},\theta|y_{1:t+1})$ by the following steps:

\begin{enumerate}
\item For each particle $i$, calculate a point estimate of $\left(x_{t+1}^{(j)},\theta\right)$ given by $\left(\mu_{t+1}^{(j)},m_k^{(j)}\right)$ where
    \[
    \mu_{t+1}^{(j)} = E\left(x_{t+1}\left|x_t^{(j)},\theta_k^{(j)} \right.\right) \qquad
    m_t^{(j)} = a\theta_t^{(j)} + (1-a)\bar{\theta}_t
    \]
\item Calculate auxiliary weights and renormalize
\[ \tilde{g}_{t+1}^{(j)} = w_k^{(j)} p\left(y_{t+1}\left|\mu_{t+1}^{(j)},m_k^{(j)}\right.\right) \qquad g_{t+1}^{(j)} = \tilde{g}_{t+1}^{(j)}\left/ \sum_{l=1}^J \tilde{g}_{t+1}^l. \right. \]
\item For each particle $j=1,\ldots,J$
	\begin{enumerate}
	\item Sample an index $k\in\{1,\ldots,j,\ldots,J\}$ with probability $g_{t+1}^{(j)}$.
...
We are interested now in how to update the posterior distribution of the states and unknown parameters given new data.  That is, given the posterior distribution at time $t_k$, denoted by $p(x_t,\theta|y_{1:k})$, how do we obtain $p(x_{t+1},\theta|y_{1:k+1})$?  Theoretically, this is obtained by Bayes rule:

\begin{equation}
p(x_{t+1},\theta|y_{1:k+1}) = \frac{p(y_{j,k}|x_t,\theta)p(x_t,\theta|y_{1:k})}{\int p(y_{j,k}|x_t,\theta)p(x_t,\theta|y_{1:k}) \mathrm{d} x_t} \label{eqn:bayes}
\end{equation}
...
\begin{figure}[ht]
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-3P-1000-beta}
...
\begin{figure}[ht]
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-3P-1000-gamma}
...
\begin{figure}[ht]
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-3P-1000-nu}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.1\textwidth]{EpidPred-KD-1000}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.1\textwidth]{Peaks-KD-1000}
...
\begin{figure}[ht]
\centering
\includegraphics[height=4in]{PF-quant-6P-10000}
...
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-beta}
\end{minipage}
\begin{minipage}[b]{1.0\linewidth}
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-gamma}
\end{minipage}
...
\centering
\begin{minipage}[b]{1.0\linewidth}
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-nu}
\end{minipage}
\begin{minipage}[b]{1.0\linewidth}
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-b}
\end{minipage}
...
\centering
\begin{minipage}[b]{1.0\linewidth}
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-varsigma}
\end{minipage}
\begin{minipage}[b]{1.0\linewidth}
\includegraphics[width=1.1\textwidth]{Hist-Time-KD-6P-10000-sigma}
\end{minipage}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.1\textwidth]{EpidPred-KD-6P-10000}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.1\textwidth]{Peaks-KD-6P-10000}
