...
An additional adjustment can be made with the KDPF by choosing to reduce the frequency with which we regenerate values of fixed parameters. We may choose not to regenerate the fixed parameters at iterations of the KDPF for which resampling is not performed, since, in this case, there is no elimination of unique fixed parameter values. At these iterations of the KDPF, we modify the `Regenerate the fixed parameters' step of the algorithm to read "$\theta_{t+1}^{(j)} = \theta_t^{(j)}$".
