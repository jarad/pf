...
Let $\bar{\theta}_t$ and $V_t$ be the weighted sample mean and weighted sample covariance matrix of $\theta_t^{(1)},\ldots,\theta_t^{(J)}$.  The KDPF uses a tuning parameter $\Delta$, the discount factor that takes values in $(0,1)$, and two derived quantities $h^2 = 1 - ((3\Delta - 1)/2\Delta)^2$ and $a^2 = 1 - h^2$ that determine how smooth the kernel density approximation is. Lower values of $\Delta$ result in a smoother approximation. However, the goal here is simply to jitter particles around to refresh values of the fixed parameters to reduce the chance of degeneracy, and so $\Delta$ is typically taken to be between 0.95 and 0.99 \citep{Liu:West:comb:2001}.
...
%Choosing a lower value for $\Delta$ may help the performance of the KDPF for lower number of particles, but \citet{Liu:West:comb:2001} recommends choosing a value between 0.95 and 0.99. We use a value of $\Delta = 0.99$ because we seek an implementation of the KDPF that performs well while requiring minimal jittering of particles. %\danny{I think the key thing is that J$<10000$ is just too small and so it doesn't make any sense to think about a $\Delta$ here. Also, I don't think we require minimal jittering, so I'm not sure I would keep the last sentence.}
