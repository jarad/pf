...
In equation \eqref{eqn:state}, $Q(\theta)$ was calculated by taking the variances and covariance of $s_t$ and $i_t$ in the discrete time approximation of the modified SIR model with stochastic fluctuations considered in \citet{skvortsov2012monitoring}, given by
...
\noindent where $\epsilon_\beta$ and $\epsilon_\gamma$ are random components with $\epsilon_\beta \sim N(0, \sqrt{\beta} / P)$ and $\epsilon_\gamma \sim N(0, \sqrt{\gamma} / P)$. The variances of these terms are derived from an established scaling law of stochastic fluctuations in population size in dynamical systems \citep{ovaskainen2010extinction, herwaarden1995stochepid, dangerfield2009stochepid}

$R_0 = \beta / \gamma$ is called the \emph{basic reproductive number}, which gives the average number of people infected by a sick person in a population where everyone is susceptible, and $R_0 > 1$ is required for an infection to spread into an epidemic. In many cases, prior information about this quantity for specific types of infections is more readily available than about $\beta$. The mixing parameter $\nu$ describes the heterogeneity of contacts within the population, where $\nu = 1$ corresponds to a population with homogenous mixing, i.e. an infectious person is equally likely to infect any susceptible, and $\nu = 0$ corresponds to a population with no social interaction. Other values of $\nu$ represent populations with heterogenous mixing \citet{stroud2006powerlaw}.
