...
\[ E\left[ \left| \int \phi (x_{t}, \theta) (\hat{p}(x_{t},\theta|y_{1:t})-p(x_{t},\theta|y_{1:t})\right|^p\right]^{1/p} \le \frac{C_t ||\phi||_\infty}{J} \] 
where $\hat{p}(x_{1:t},\theta|y_{1:t})$ is the particle approximation given by equation \eqref{eqn:filtered}, $||\cdot||_\infty$ is the infinity norm, and $C_t$ is an increasing function of $t$ \citep{del2004feynman}. The key here is that for a fixed $t$, we can increase the number of particles $J$ to reduce our Monte Carlo error. But this is true for all of the algorithms above including the BF and APF, so the question is what does $C_t$ look like for each algorithm. This is a hard question to answer for general models. Instead, in Section \ref{sec:results}, we compare performance of these algorithms at various values for $J$ within a specific model. 
...
\danny{the next paragraph needs to be reworked a bit, I think}
...
We suggest log-normal priors on positive elements of $\theta$ and because it allows us to use prior knowledge of the epidemic to encourage points to lie in a reasonable range, but is more flexible in the event of model mis-specification either in the likelihood or the prior and allows the data to play a greater role in dictating the evolution of $p(x_t,\theta|y_{1:t})$.
