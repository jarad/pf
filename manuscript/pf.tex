...
Figure \ref{fig:mcmc} compares the PMCMC results with 20 separate runs of the KDPF on the same simulated data using our log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$. Multiple runs of the KDPF on the same data allows us to assess the uncertainty in the 95\% credible intervals obtained from the filtered distributions at each $T$. For instance, the high variance of the interval estimates for $\nu$ in the rightmost panel of the figure demonstrates the challenge in estimating this parameter. Nonetheless, the performance of the KDPF in this case compares well with PMCMC, as the lower and upper bounds of the 95\% credible intervals obtained from PMCMC fall within the cloud of intervals obtained from the separate KDPF runs for most of the epidemic. Furthermore, the PMCMC on the full data set took 8-9 hours to run while the KDPF with $J = 20000$ particles provided results in about 15 minutes.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{sir-pmcmc-kdpf-1-20000-20-20000}
\caption{Sequential 95\% credible intervals obtained every fifth day over 20 runs of the KDPF (green lines) using 20000 particles, log-normal priors on $\theta$, stratified resampling, and $\Delta = 0.99$ compared with 95\% credible intervals obtained from separate PMCMC analyses (black circles) on observations from the same simulated data set observed up until time $T$ for $T = 5, 10, 15, \ldots, 125$.} \label{fig:mcmc}
\end{figure}
...
The KDPF provides a generic sequential inferential strategy that reduces particle degeneracy in many situations. However, the algorithm is known to perform poorly under certain conditions. For instance, as mentioned in Section \ref{sec:pf}, all SMC methods suffer from degeneracy if vague priors are used. A common solution to this problem is to first run an MCMC based on the first few data points to find a reasonable particle cloud from which to draw prior samples \citep{petris2009dynamic}. In addition, the KDPF does not perform well in high-dimensional parameter space and, like all SMC methods, degrades over time. PMCMC methods can provide better estimates of unknown states and fixed parameters in high dimensions by using SMC methods to construct efficient proposal distributions. Since it is likely that continuous monitoring using more complicated models than what we presented in this paper may be required for monitoring disease outbreaks in real-life situations, we recommend that PMCMC be used as long as the time-scale of the epidemic allows for it. However, a sequential analysis could be valuable for processing data collected on a time scale of hours, minutes, or seconds.
