...
\includegraphics[width=1.0\textwidth]{sim-orig-epid-3}
\end{minipage}
\begin{minipage}{0.48\linewidth}
\includegraphics[width=1.0\textwidth]{sim-orig-z-3}
\end{minipage}
...
The behavior of the BF, APF, and KDPF shown in Figure \ref{fig:pfs} is consistent across the 40 simulations. For instance, for $J = 20000$, the 95\% credible intervals at $t = 125$ for each of $\beta$, $\gamma$ and $\nu$ cover the truth for 38 out of 40 simulations using the KDPF. For the BF and APF using the same number of particles, on the other hand, the maximum number of simulations for which 95\% credible intervals at $t = 125$ cover the truth for any of the unknown parameters is 14.
...
\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{PF-3-systematic-unif-logit-99-61-params}
...
\includegraphics[width=1.0\textwidth]{PF-betaGammaScat-1-10000-KD-systematic-unif-logit-99-61}
\vspace{-1.0cm}
...
\begin{minipage}{1.0\linewidth}
\includegraphics[width=1.0\textwidth]{PF-betaGammaScat-1-10000-KD-systematic-unif-log-99-61}
\vspace{-1.0cm}
...
\begin{minipage}{1.0\linewidth}
\includegraphics[width=1.0\textwidth]{PF-betaGammaScat-1-10000-KD-systematic-orig-log-99-61}
\vspace{-1.0cm}
...
In theory, 50\% credible intervals calculated from the true posterior distribution should cover the true parameter value about half the time. Figure \ref{fig:resamp} shows that coverage approaches the nominal level for all four resampling techniques as $J$ increases. However, multinomial resampling appears to be outperformed by the other three resampling techniques.  For example, using multinomial resampling and $J = 1000$ particles, the coverage level for $\beta$ and $\gamma$ lies below the lower 95\% confidence bound during the second half of the epidemic while coverage levels remain above the bound for other three methods. Looking at the same row of Figure \ref{fig:resamp}, we also see they residual and stratified resampling give slightly higher coverage levels than does systematic resampling. Due to this as well as an example shown in \cite{Douc:Capp:Moul:comp:2005} where systematic resampling has more Monte Carlo variability than any of the other three resampling schemes, we prefer to use either residual or stratified resampling.

\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{PF-coverage-5-20-KD-orig-resamp-params}
\caption{Proportion of simulations for which 50\% credible intervals for the three model parameters (columns) cover the true value used for simulation for different $t$ (x-axis) and $J$ (rows) using the KDPF with log-normal priors on $\theta$ and $\Delta = 0.99$. Solid gray horizontal line denotes nominal coverage (50\%) and dashed lines give 95\% confidence bounds around the true coverage.} \label{fig:resamp}
\end{figure}
