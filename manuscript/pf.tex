...
Practical implementation of any particle filter requires the practitioner to make several choices, one of which being what type resampling scheme to use. Throughout our discussion, we have explicitly used multinomial resampling, but alternative resampling schemes exist including stratified, residual, and systematic resampling \citep{Douc:Capp:Moul:comp:2005}. Resampling is meant to rebalance the weights of the particles in order to avoid degeneracy, but this comes at the cost of adding additional Monte Carlo variability to the particle sample. This cost can be understood by considering the situation where no data are observed and multinomial resampling is performed at every iteration of the particle filter. In this case, our knowledge about the states and unknown parameters should not change over time, but we in fact lose information about the filtered distribution by sampling from the particles with replacement. The four resampling techniques just mentioned may add different amounts of Monte Carlo variability to the particle sample in a given situation. In Section \ref{sec:resample}, we suggest the use of stratified or residual sampling.

In addition, the frequency of resampling should be reduced to balance the loss of information due to degeneracy with the loss of information due to the additional Monte Carlo variability introduced during resampling. Typically, a measure of the nonuniformity of particle weights is used to determine if resampling should be performed at a given iteration of a particle filter. We use effective sample size \citep{Liu:Chen:Wong:reje:1998}, a value that can be interpreted as the number of independent particle samples, ranging between 1 and $J$, with $J$ corresponding to all particle weights being equal and 1 corresponding to one particle weight being 1 with the rest 0. Using this measure of nonuniformity in our runs, we set a threshold of 0.8, meaning that if the number of independent samples is less than 80\% of the total number of particles, resampling is performed. Other measures of nonuniformity include entropy and coefficient of variation, and future work is needed to determine which choice of nonuniformity measure and threshold level are optimal in certain cases.

The BF, APF, and KDPF algorithms described in the previous sections were constructed under the assumption that resampling is performed at every iteration of the filter. At iterations for which resampling is not performed, we modify the algorithms by 1) omitting the `Resample' step, 2) replacing all instances of the sampled index $k$ with the particle index $j$, and 3) adjusting the calculation of $\tilde{w}_{t+1}^{(j)}$ by multiplying by $w_t^{(j)}$.

An additional adjustment can be made with the KDPF by choosing to reduce the frequency with which we regenerate values of fixed parameters. We may choose to not regenerate the fixed parameters for iterations of the filter at which resampling is not performed, since, in this case, there is no elimination of unique fixed parameter values. At these iterations of the KDPF, we modify the `Regenerate the fixed parameters' step of the algorithm to read "$\theta_{t+1}^{(j)} = \theta_t^{(j)}$".
...
We now compare the performance of the BF, APF and KDPF using simulated data analogous to that used in \citep{skvortsov2012monitoring}. In addition, we compare the performance of the KDPF when using bounded versus unbounded priors on the fixed parameters and under different resampling schemes.
