...
Systematic resampling and an effective sample size was used to determine when to resample particles.  For the KDPF, the tuning parameter $\delta$ was set to 0.99 to determine the variance of the normal kernel density approximation of $p(\theta|y_{1:t})$.
...
%To investigate the impact of different prior distributions of $\theta$ on performance of the particle filters, the runs described above were done once using uniform priors on $\theta$ and then again using normal priors.  

Uniform priors on $p(\beta)$, $p(\gamma)$, and $p(\nu)$ are given by
